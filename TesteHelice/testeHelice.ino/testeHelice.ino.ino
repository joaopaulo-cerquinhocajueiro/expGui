...
// variável volátil para ser acionada pela interrupção
...
#define Ta 20000
float pulseCounterConstant = 1e6/(2.0*Ta);
// 60 segundos por minuto
// 1e6 microsegundos/segundo
// 2 hélices - 2 pulsos por volta
...
// Interrupção do contador
  ...
  attachInterrupt(digitalPinToInterrupt(interruptPin), IncreaseCounter, RISING);
...
//        vout = pid(vin,setPoint,Kp,Ki,Kd,vout,intVout);
      ...
      int pulseFreq = pulseCounterConstant*pulse_counter;
      Serial.write(byte(pulseFreq>>8));
      Serial.write(byte(pulseFreq%256));
      ...
      if(++expNumber>T1){
        estado = parado;
        brushless.write(40); // return to 0
      }
    }
    break;
  case erro:
